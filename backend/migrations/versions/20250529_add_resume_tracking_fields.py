...
Revision ID: 20250529_resume_tracking
Revises: 20250417_add_embeddings
...
revision = '20250529_resume_tracking'
down_revision = '20250417_add_embeddings'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    # Create GIN indexes on parsed_data column (casting to JSONB)
    op.execute('CREATE INDEX IF NOT EXISTS idx_parsed_data_email ON resumes USING gin ((parsed_data::jsonb))')
    op.execute('CREATE INDEX IF NOT EXISTS idx_parsed_data_skills ON resumes USING gin ((parsed_data::jsonb))')
